...
export async function POST(request) {
  const rate = applyRateLimit(request, "auth-login", { limit: 20, windowMs: 60_000 });
  if (!rate.allowed) {
    return NextResponse.json({ error: "Too many login requests. Try again later." }, { status: 429 });
  }
  ...
  const result = await authApi.login(body, getRequestContext(request));
